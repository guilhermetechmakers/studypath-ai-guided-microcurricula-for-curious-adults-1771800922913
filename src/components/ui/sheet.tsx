...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn('fixed inset-0 z-50 bg-black/50 transition-opacity duration-300', className)}
    {...props}
  />
))
...
const sheetVariants: Record<SheetSide, string> = {
  right:
    'inset-y-0 right-0 h-full w-full sm:max-w-lg animate-slide-in-right',
  left:
    'inset-y-0 left-0 h-full w-full sm:max-w-lg',
  top:
...
>(({ side = 'right', showClose = true, className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed z-50 gap-4 border bg-card shadow-lg transition ease-in-out',
        sheetVariants[side],
        side === 'right' || side === 'left' ? 'flex flex-col' : '',
        className
      )}
      {...props}
    >
